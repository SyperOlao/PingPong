...
Изначально это была простая Pong-игра, но давно переросла в небольшой игровой движок на `C++20` и `DirectX 11`: общее `Core`, единая точка входа, общие сервисы приложения и несколько отдельных мини-игр/демо, работающих поверх одной базы. Собранное приложение — `MiniEngineDemo.exe`, внутри которого по-прежнему есть режим Pong среди других демо.
...
- В проекте уже есть forward/deferred rendering, освещение и каскадные тени.
...
В `main.cpp` через `enum class DemoType` выбирается, какая игра будет создана:
...
Классическая 2D-игра, с которой проект когда-то начинался, но теперь это один из режимов движка.
...
- follow-camera;
- HUD со статистикой;
- перезапуск уровня и debug-режимы.
...
Сейчас проект собирается как одно приложение, а конкретная игра выбирается в `main.cpp`.
