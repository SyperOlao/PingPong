...
void Timer::Tick()
{
    ...
    const long long elapsedCounts = m_currentCounter - m_previousCounter;
    m_deltaTime = static_cast<float>(
        static_cast<double>(elapsedCounts) / static_cast<double>(m_frequency)
    );
...
{
    const long long elapsedCounts = m_currentCounter - m_startCounter;
